...
impl AdcShim {
    pub fn new(
        ...
        porte_ams
            .dir
            .write(|w| unsafe { w.bits((porte_ams.dir.read().bits() & !0x003F)) });
        porte_ams
            .afsel
            .write(|w| unsafe { w.bits((porte_ams.afsel.read().bits() | 0x003F)) });
        porte_ams
            .den
            .write(|w| unsafe { w.bits((porte_ams.den.read().bits() & !0x003F)) });
        porte_ams
            .amsel
            .write(|w| unsafe { w.bits((porte_ams.amsel.read().bits() | 0x003F)) });

        ad0.pc
            .write(|w| unsafe { w.bits((ad0.pc.read().bits() & !0x0F)) });
        ad0.pc
            .write(|w| unsafe { w.bits((ad0.pc.read().bits() | 0x01)) });
        ad0.sspri.write(|w| unsafe { w.bits(0x0123) });
        ad0.actss
            .write(|w| unsafe { w.bits((ad0.actss.read().bits() & !0x0008)) });
        ad0.emux
            .write(|w| unsafe { w.bits((ad0.emux.read().bits() & !0xF000)) });
        ad0.ssmux3
            .write(|w| unsafe { w.bits((ad0.ssmux3.read().bits() & !0x000F)) });
        ad0.ssmux3
            .write(|w| unsafe { w.bits((ad0.ssmux3.read().bits() + 9)) });
        ad0.ssctl3.write(|w| unsafe { w.bits(0x06) });
        // ad0.ssctl3.write(|w| unsafe{w.bits(0x06)});
        //  ad0.sspri.write(|w| unsafe{w.bits(0x0123)});
        ad0.im
            .write(|w| unsafe { w.bits((ad0.im.read().bits() & !0x0008)) });
        Self {
            states: PinArr([State::Disabled; Pin::NUM_PINS]),
                ...
                match x {
                    0 => {
                        ad0.ssmux3
                            .write(|w| unsafe { w.bits((ad0.ssmux3.read().bits() & !0x000F)) });
                        ad0.ssmux3
                            .write(|w| unsafe { w.bits((ad0.ssmux3.read().bits() + 0)) });
                    }
                    1 => {
                        ad0.ssmux3
                            .write(|w| unsafe { w.bits((ad0.ssmux3.read().bits() & !0x000F)) });
                        ad0.ssmux3
                            .write(|w| unsafe { w.bits((ad0.ssmux3.read().bits() + 1)) });
                    }
                    2 => {
                        ad0.ssmux3
                            .write(|w| unsafe { w.bits((ad0.ssmux3.read().bits() & !0x000F)) });
                        ad0.ssmux3
                            .write(|w| unsafe { w.bits((ad0.ssmux3.read().bits() + 2)) });
                    }
                    3 => {
                        ad0.ssmux3
                            .write(|w| unsafe { w.bits((ad0.ssmux3.read().bits() & !0x000F)) });
                        ad0.ssmux3
                            .write(|w| unsafe { w.bits((ad0.ssmux3.read().bits() + 3)) });
                    }
                    4 => {
                        ad0.ssmux3
                            .write(|w| unsafe { w.bits((ad0.ssmux3.read().bits() & !0x000F)) });
                        ad0.ssmux3
                            .write(|w| unsafe { w.bits((ad0.ssmux3.read().bits() + 8)) });
                    }
                    5 => {
                        ad0.ssmux3
                            .write(|w| unsafe { w.bits((ad0.ssmux3.read().bits() & !0x000F)) });
    ...
    fn read(&self, pin: Pin) -> Result<u8, ReadError> {
        ...
        match self.states[pin] {
            Enabled(value) => {
                ...
                let out = p.isc.read().bits() & 0x0FFF;
