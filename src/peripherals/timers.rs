...
use tm4c123x_hal::{time::*, timer, timer::Timer, timer::*};
    ...
    fn default() -> Self {
        ...
        sc.clock_setup.oscillator = tm4c123x_hal::sysctl::Oscillator::Main(
            ...
            tm4c123x_hal::sysctl::SystemClock::UsePll(
                tm4c123x_hal::sysctl::PllOutputFrequency::_20mhz,
            ),
        );
...
impl TimersShim<'_> {
    pub fn new(
        system_config: &'static tm4c123x_hal::sysctl::Sysctl,
        frozen_clock: &'static tm4c123x_hal::sysctl::Clocks,
    ...
    fn set_period(&mut self, timer: TimerId, milliseconds: Word) -> Result<(), TimerMiscError> {
        ...
        match timer {
            T0 => {
                ...
                t0.ctl
                    .modify(|_, w| w.taen().clear_bit().tben().clear_bit());
                t0.tav.write(|w| unsafe {
                    w.bits(millis_to_ticks(milliseconds as f32, self.clock_freq as f32))
                });
            ...
            T1 => {
                ...
                t1.ctl
                    .modify(|_, w| w.taen().clear_bit().tben().clear_bit());
                t1.tav.write(|w| unsafe {
                    w.bits(millis_to_ticks(milliseconds as f32, self.clock_freq as f32))
                });
                Peripherals::take().unwrap().TIMER1.tailr.write(|w| unsafe {
                    w.bits(millis_to_ticks(milliseconds as f32, self.clock_freq as f32))
                });

                // // start counter
                t1.ctl.modify(|_, w| w.taen().set_bit());
...
fn scratch() {
    ...
    t.ctl.modify(|_, w| w.taen().clear_bit().tben().clear_bit());
